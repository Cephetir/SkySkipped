...
object SBInfo {
    ...
    fun onTick(event: TickEvent.ClientTickEvent) {
        if (event.phase != TickEvent.Phase.START || mc.thePlayer == null || mc.theWorld == null || !Cache.inSkyblock) return
        ...
        try {
            val lines = ScoreboardUtils.sidebarLines.map { it.stripColor() }
            if (lines.size >= 5) {
                //§707/14/20
