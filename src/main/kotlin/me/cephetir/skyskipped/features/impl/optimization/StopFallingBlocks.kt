...
class StopFallingBlocks : Feature() {
    init {
        listener<PacketEvent.Receive> {
            if (!Config.stopFallingBlocks.value || !SkyblockListener.onSkyblock || it.packet !is S0EPacketSpawnObject || (it.packet as S0EPacketSpawnObject).type != 70) return@listener
            it.cancel()
        }
    }
}